...
class Messanger:
    ...
    async def wait_message(self) -> message_entity.Message | None:
        ...
        if not self._subscription_started:
            raise Exception("Not subscribed. Open context manager to start subscription.")
        ...
        async with self.uow as repository:
            await repository.change_status(message.message_id, message_entity.MessageStatus.DELIVERED)
            await repository.commit()

        return message

    async def mark_as_received(self, receiver: typing.Text, message_id: uuid.UUID) -> None:
        """
        ...
        async with self.uow as repository:
            ...
            if message.receiver != receiver:
                raise exceptions.ChangeStatusAccessDonated(receiver, message_id, message_entity.MessageStatus.RECEIVED)

            await repository.change_status(message_id, message_entity.MessageStatus.RECEIVED)
            await repository.commit()
    ...
    async def mark_as_read(self, reader: typing.Text, message_id: uuid.UUID) -> None:
        ...
        async with self.uow as repository:
            ...
            if message.receiver != reader:
                raise exceptions.ChangeStatusAccessDonated(reader, message_id, message_entity.MessageStatus.READ)

            await repository.change_status(message_id, message_entity.MessageStatus.READ)
            await repository.commit()
    ...
    async def delete_message(self, deleter: typing.Text, message_id: uuid.UUID) -> None:
        ...
        async with self.uow as repository:
            ...
            if deleter not in (message.receiver, message.sender):
                raise exceptions.ChangeStatusAccessDonated(deleter, message_id, message_entity.MessageStatus.DELETED)

            await repository.change_status(message_id, message_entity.MessageStatus.DELETED)
            await repository.commit()
